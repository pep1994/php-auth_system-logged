<?php
    ...
    class AuthSys extends Secure\AuthSysSecure
    {
        ...
        {
            $formati_ammessi = ['pdf', 'png', 'doc', 'jpg'];
            $dim_max = 2000000;
    
            foreach ($file['upl']['name'] as $key => $value) {
                $error = [];
                $check = true;
                // controllo se è stato caricato il file
                if ($file['upl']['error'][$key] == UPLOAD_ERR_NO_FILE) {
                    return;
                }
                // controllo che non ci siano errori d icaricamento
                if ($file['upl']['error'][$key] != UPLOAD_ERR_OK) {
                    $error[] = "Errore nel caricamento del file " . $file['upl']['name'][$key];
                    $check = false;
                }
                // controllo che la dimensione del file non superi i 2MB
                if ($file['upl']['size'][$key] > $dim_max) {
                    $error[] = "Il file " . $file['upl']['name'][$key] . " supera le dimensioni consentite (MAX 2MB)";
                    $check = false;
                }
                // controllo che il formato del file sia tra quelli ammessi
                $est = strtolower(pathinfo($file['upl']['name'][$key], PATHINFO_EXTENSION));
                if (!in_array($est, $formati_ammessi)) {
                    $error[] = "Il file " . $file['upl']['name'][$key] . " ha formato $est, il quale non è consentito";
                    $check = false;
                }
                if ($check === true) {
                    // controllo che il file venga spostato correttamente nella cartella
                    $nome_file = bin2hex(random_bytes(20)) . "." . $est;
                    if (!move_uploaded_file($file['upl']['tmp_name'][$key], "uploads/$nome_file")) {
                        $error[] = "Non è stato possibile effettuare lo spostamento del file " . $file['upl']['name'][$key];
                        $check = false;
                    }
                }
                if ($check === true) {
                    echo "<p style='color: green'> Il file " . $file['upl']['name'][$key] . " è stato caricato con successo</p>";
                } else {
